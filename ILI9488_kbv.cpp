...
#define ILI9488_CMD_COLOR_SET                       0x2D
...
uint16_t ILI9488_kbv::readReg(uint16_t reg)
{
    uint8_t h, l;
    CS_ACTIVE;
    WriteCmd(reg);
...
void ILI9488_kbv::drawPixel(int16_t x, int16_t y, uint16_t color)
{
    ...
    spibuf[1] = (color >> 3);
    spibuf[2] = (color << 3);
	pushCommand(ILI9488_CMD_MEMORY_WRITE, spibuf, 3);
#endif
}
...
void ILI9488_kbv::setAddrWindow(int16_t x, int16_t y, int16_t x1, int16_t y1)
{
    ...
    spibuf[2] = x1 >> 8;
    spibuf[3] = x1;
	pushCommand(ILI9488_CMD_COLUMN_ADDRESS_SET, spibuf, 4);
    spibuf[0] = y >> 8;
    spibuf[1] = y;
    spibuf[2] = y1 >> 8;
    spibuf[3] = y1;
	pushCommand(ILI9488_CMD_PAGE_ADDRESS_SET, spibuf, 4);
}
...
#define TFTLCD_DELAY8 0xFF
        static const uint8_t ILI9488_RGB_regValues[] PROGMEM = {    // 320x480
            0x01, 0,            //Soft Reset
            TFTLCD_DELAY8, 125,
            0x11, 0,            //Sleep Out
            TFTLCD_DELAY8, 20,
...
void ILI9488_kbv::begin(uint16_t ID)
{
    _lcd_ID = ID;
	_lcd_xor = 0x02;
    uint8_t *p = (uint8_t *)ILI9488_RGB_regValues;
    int16_t size = sizeof(ILI9488_RGB_regValues);
    reset();
    while (size > 0) {
